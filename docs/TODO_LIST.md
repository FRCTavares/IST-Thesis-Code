...
4. [ ] [#25 — P1.10 Improve bbox evaluation](https://github.com/FRCTavares/IST-Thesis-Code/issues/25) — HIGHEST PRIORITY
   ...
   - resumed on 25 August on `issue-25-bbox-evaluation-continuation-20260825`, based on current `origin/main` while preserving the parked `issue-25-improve-bbox-evaluation` checkpoint. The v1/v2 physical-reference contracts, identity-independent evaluators, and v2 annotation workspace are present; M3-v2 acceptance re-verification and the final-semantics M4A-v2 workload re-plan remain before human annotation. No canonical physical-reference annotation or v2 evaluation evidence exists yet.
