...
6. [ ] [#44 — P1.14+ ReID placement: select on CPU, then promote the winner to Hailo (refines P1.14 + Deferred ReID/Hailo items)](https://github.com/FRCTavares/IST-Thesis-Code/issues/44) — IN PROGRESS
    ...
    - A standalone pure CPU candidate-request policy now defines the unchanged `all_candidates` baseline and the experimental `geometry_winner` mode. It uses only public target state, pending operator selection, and stateless geometry scoring; runtime/backend wiring remains pending.
