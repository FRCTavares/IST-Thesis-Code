...
1. [ ] [#50 — Complete flight readiness and retained UAV closed-loop evidence](https://github.com/FRCTavares/IST-Thesis-Code/issues/50) — FIELD REQUIRED
   ...
   - 25 September operator-sheet consolidation: the canonical `docs/flight/field_day_runbook.md` now includes the existing manual-RC moving-UAV TIM-MARS observation as Flight 0 and explicitly enumerates the six frozen matched baseline/candidate flights as `B C | C B | B C`, with exact per-flight tags and copy-paste event/verification commands. This is documentation/operator hardening only; no controller, TIM-MARS, recovery, recorder acceptance, network, or safety criterion changed.
