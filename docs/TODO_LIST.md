...
5. [ ] [#58 — P1.13+ Compare lightweight tracker + TIM-MARS against integrated appearance-aware tracking](https://github.com/FRCTavares/IST-Thesis-Code/issues/58) — HIGHEST PRIORITY — IN PROGRESS
   ...
   - 31 Aug 2026: the canonical embedded-cost development comparison for Issue #58 is now complete on Raspberry Pi 5 at commit `92d123e3` using the same June Seq03 source, direct-Hailo YOLOv8s, real-time `1.0x` playback, three repetitions per architecture, rotated execution order, and 1 s process-group/hardware sampling. All 9/9 retained cells completed with zero throttling and zero hardware-sampler errors. Architecture-specific run-level means (common detector excluded) are: ByteTrack raw `227.2 +/- 0.6%` CPU and `134.6 +/- 0.2 MiB` mean RSS; ByteTrack + TIM-MARS `243.3 +/- 10.3%` CPU and `919.6 +/- 7.8 MiB` mean combined tracker+TIM RSS; DeepSORT raw `267.0 +/- 3.0%` CPU and `767.2 +/- 1.9 MiB` mean RSS. Thus the selective ByteTrack + TIM-MARS architecture used about `8.9%` less architecture CPU than DeepSORT on this controlled development workload but about `19.9%` more mean architecture RSS. Full perception-system means including the common detector are approximately `254.5% / 353.1 MiB`, `267.8% / 1138.6 MiB`, and `291.9% / 985.5 MiB` for ByteTrack raw, ByteTrack + TIM-MARS, and DeepSORT raw respectively. Mean SoC temperatures were `60.8 C`, `63.1 C`, and `65.0 C`; highest observed temperatures were `63.1 C`, `67.0 C`, and `69.2 C`. Do not yet promote timing-topic message-count differences into a throughput claim: `/timing_tracker` and `/timing_target` distributions must be aggregated from the retained bags first. Issue #58 remains OPEN; held-out H01--H03 physical-v2 evaluation under Issue #27 is still required before the final architecture claim. Do not rerun the frozen May/Seq03/Seq04 safety matrices or add StrongSORT/BoT-SORT unless the minimum comparison proves scientifically insufficient.
