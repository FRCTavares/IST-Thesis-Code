...
9. [ ] [#55 — P1.22 Repair and test the live UI launch, build, and access-control contract](https://github.com/FRCTavares/IST-Thesis-Code/issues/55)
    ...
    - 2 Sep 2026 Pixhawk-gated field-network correction in progress: the required invariant is `NO PIXHAWK -> NO AERONEXT GCS`, both before field entry and continuously after entry. The final recovery audit also removed generic unattended `nmcli device connect wlan0` recovery; unattended recovery now only re-enables NetworkManager device autoconnect, so field/GCS profiles persisted with `connection.autoconnect=no` cannot be manually selected by the health backstop. The same final audit removed automatic field-Wi-Fi reconnects from the health monitor: any field-network contract loss now exits to `unattended`, and only a new explicit `pixhawk` command may activate an approved GCS profile. Configured field/GCS profiles are persisted with `connection.autoconnect=no`; explicit `pixhawk` entry requires physical carrier before field-WiFi activation and verifies the active `pixhawk-apm` profile owns no default route before Tailscale may be disabled. Continuous enforcement adds a NetworkManager link-down dispatcher as the fast host-only exit path plus carrier/profile/default-route validation in `thesis_host_health.py` as an independent periodic backstop. A confirmed field-link loss returns to `unattended`, explicitly drops active field Wi-Fi, relinquishes Pixhawk Ethernet, restores ordinary NetworkManager autoconnect and Tailscale, and never performs automatic field re-entry when carrier returns. M6 remains OPEN pending static validation, installation, live no-Pixhawk negative testing, positive connected-Pixhawk testing, physical disconnect testing, and final checklist/evidence reconciliation.
