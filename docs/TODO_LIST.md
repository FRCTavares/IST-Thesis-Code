...
1. [ ] [#64 — Resolve high-resolution appearance crops on representative small-target UAV footage](https://github.com/FRCTavares/IST-Thesis-Code/issues/64) — FIELD DEPENDENT / #32 PREREQUISITE
   ...
   - Time-constrained operator decision on 22 September: after valid formal Cell 1, prioritize a matched HD ByteTrack+TIM comparison before the remaining matrix cells. HD structured-visual capture is permitted at 1280x720/30 Hz with the same frozen exposure settings. Treat this out-of-order Cell 5 capture as a documented protocol deviation and do not claim the full eight-cell matrix is complete.
