...
6. [ ] [#44 — P1.14+ ReID placement: select on CPU, then promote the winner to Hailo (refines P1.14 + Deferred ReID/Hailo items)](https://github.com/FRCTavares/IST-Thesis-Code/issues/44) — IN PROGRESS
    ...
    - The pure CPU candidate-request policy is now wired into the shared runtime through an encoder-only request mask. The complete candidate list remains authoritative for crop quality, cache reuse, and tracker lifecycle accounting. `all_candidates` remains the unchanged default; ROS parameter, status, and deterministic-replay plumbing for controlled experiments remains pending.
