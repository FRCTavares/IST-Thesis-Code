...
5. [ ] [#58 — P1.13+ Compare lightweight tracker + TIM-MARS against integrated appearance-aware tracking](https://github.com/FRCTavares/IST-Thesis-Code/issues/58) — HIGHEST PRIORITY — IN PROGRESS
   ...
   - 31 Aug 2026: the canonical pre-#58 Issue #32 instrumentation gate is complete. The remaining development comparison is narrowed to the embedded-cost axis for ByteTrack raw, ByteTrack + TIM-MARS, and DeepSORT raw on one controlled development source. The full-pipeline runner now gains only opt-in reuse of the existing process-group CPU/RSS and hardware-health samplers; normal replay behaviour remains unchanged by default. The first plumbing smoke passed process-group resolution, CPU/RSS collection, hardware-health collection, and zero-descendant cleanup, while exposing that sampler shutdown initially included the recorder's post-playback stop wait; the measurement window is therefore bounded explicitly to active replay before retained cost runs. The first retained-matrix attempt then exposed one runner-only raw-cell defect: `tim=off` still attempted the TIM-MARS selected-target authority bootstrap and correctly received HTTP 503 because no TIM-MARS subscriber exists. Raw ByteTrack/DeepSORT cost cells do not require selected-target authority, so target resolution/selection is now conditional on TIM-MARS being enabled. Do not rerun the frozen May/Seq03/Seq04 safety matrices or add StrongSORT/BoT-SORT unless the minimum comparison proves scientifically insufficient.
