...
1. [ ] [#90 — Add long-gap global appearance-based target reacquisition](https://github.com/FRCTavares/IST-Thesis-Code/issues/90) — TEST-FIRST IMPLEMENTATION IN PROGRESS
   - Issue #89 is closed and has frozen the comparison-eligible versus memory-update-eligible appearance contract. The first #90 deterministic implementation slice is GREEN: prolonged LOST can use protected identity for far/new-ID recovery without stale last-bbox geometry gating appearance, while distractor-only and insufficient-margin scenes remain LOST. The 13-test deterministic algorithm contract, ROS/configuration plumbing, full thesis_bringup regression suite, and package build are now GREEN. Proposal-source provenance and per-proposal candidate counts are now exposed in TIM status evidence, allowing deterministic replays to distinguish global-reacquisition attempts from existing recovery paths without changing algorithm behaviour; canonical enablement remains false while development replay/evidence and the final freeze remain pending. Short-gap recovery stays unchanged and H01--H03 remain untouched.
