...
1. [ ] [#89 — Fix wide-crop appearance eligibility blocking TIM-MARS reacquisition](https://github.com/FRCTavares/IST-Thesis-Code/issues/89) — SECOND SOFTWARE FIX READY — FRESH LIVE REPRODUCTION PENDING
   ...
   - first software slice frozen at `046ff8b3`: wide crops may now provide current comparison appearance while `appearance_crop_max_aspect_ratio=1.00` remains a positive-memory trust gate. Same-ID hijack protection is unchanged and no geometry-only fallback was added. `thesis_bringup` build passed; pytest recorded `425 passed, 1 skipped`; flake8 and `git diff --check` passed.
   - 4 Sep live validation exposed a second, downstream contract violation: a same-ID wide crop at about `1.37` aspect ratio was successfully encoded, produced current MARS appearance (`0.8435` similarity from `trusted_gallery`), and remained positive-memory-ineligible, but `protected_gallery_reacquisition_reject_reason()` still rejected it as `untrusted_crop` solely because `appearance_memory_update_eligible=false`. The focused correction now permits protected-gallery comparison only when explicit current crop-quality provenance is encoding-eligible and its sole memory-trust rejection is `aspect_ratio_too_wide`; unknown, overlapping, group-contaminated, clipped, undersized, or otherwise unsafe crops remain rejected. Test-first RED/GREEN validation passed, the full `thesis_bringup` suite records `427 passed, 1 skipped`, and the package build plus `git diff --check` pass. A fresh live restart is still required before #89 may close.
