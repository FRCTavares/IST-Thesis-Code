...
5. [ ] [#64 — P1.9+ Evaluate higher-resolution source frames for appearance crops while retaining 640x640 Hailo detection](https://github.com/FRCTavares/IST-Thesis-Code/issues/64) — HIGHEST PRIORITY
   - controlled replay infrastructure is ready on the isolated Issue #64 branch: alternate appearance-only bags are exact-timestamp/provenance checked, master-coordinate tracker evidence is guarded by a deterministic digest, and upsampled controls cannot be mistaken for high-resolution evidence. A 640x360/1280x720 VisDrone development smoke passed with frozen candidates; this is not final evidence. The repository-derived 1920x1080 TEVS field-capture procedure is prepared in `docs/flight/p064_high_resolution_capture_runbook.md`, with separate controlled-ground and UAV-motion protocols, explicit proof that the detector remains 640x640, native raw-image verification, provenance checks, and conservative storage gates. Genuine TEVS high-resolution capture, the controlled matrix, live cost measurements, and physical-reference evaluation remain pending.
