...
12. [ ] [#25 — P1.10 Improve bbox evaluation](https://github.com/FRCTavares/IST-Thesis-Code/issues/25) — HIGHEST PRIORITY
   ...
   - Branch `issue-25-improve-bbox-evaluation` active. Milestone 1 (physical-reference annotation/scoring contract) landed and corrected on review: docs/issues/p1-10-improve-bbox-evaluation.md freezes the tim_physical_target_bbox_v1 schema (identity_context: target_only/distractors_complete) and a threshold-free Stage A identity rule that never lets poor localisation of the correct person masquerade as a wrong-person or unresolved outcome; tools/analysis/physical_target_reference.py is the deterministic schema validator plus the tracker-ID-independent Stage A classifier, with 42 focused tests. Milestone 2 landed: tools/analysis/physical_target_bbox_evaluation.py is the executable identity-independent evaluation core (timebase join, Stage A/B, reconciling duration buckets, aggregation) and tools/analysis/evaluate_physical_target_bbox.py is its CLI, introduced alongside (not replacing) the legacy tracker-ID evaluator, with 35 further focused tests (77 total for #25), all proven against synthetic data. Milestone 3 landed: tools/bag_annotation_ui/ gained a third "Physical reference (Issue #25)" workspace mode (new tim_physical_reference_ui.js + tim_ui_physical_reference.py backend adapter, both additive to the existing tracker-ID annotation UI, which is unmodified and re-verified against the live server) -- pointer-driven bbox drawing mapped deterministically to source-image pixels, state/context controls enforcing the frozen schema client-side (backend validator remains authoritative), and save/load through physical_target_reference.py with no duplicated schema logic, with 17 further focused tests (94 total for #25), corrected after a browser smoke test found the coordinate-convention control could silently default to the modern P53 contract for a historical May/June bag and the save path/identity fields were too easy to leave as smoke-test placeholders: coordinate_convention is now auto-resolved deterministically (never silently defaulting either way) and sequence_id is now a required non-empty field, with 8 further tests (102 total for #25); a further corrective follow-up fixed the same navigation flow to also reset the interpolate_from_previous checkbox on unsaved frames rather than leaving it inherited from whatever frame was previously shown, with 13 further tests (115 total for #25, HEAD eb4db17a). Milestone 4A (planning only, no code) landed: docs/issues/p1-10-physical-reference-annotation-plan.md is the manual annotation workload plan for the four canonical sequences -- exact raw `/camera/image_raw` bag paths (resolving one genuine ambiguity in June Seq01's two candidate raw bags by direct visual inspection), per-sequence timeline regions classified target-only-interpolatable vs distractors-complete vs absent/reference-unavailable from direct frame sampling (existing tracker-ID CSVs used only as navigation guidance, confirmed non-transferable in three of four cases), and an estimated ~643 expected / ~1260 conservative-upper-bound manual drawing actions across all four sequences combined (~10% of total source frames). A corrective re-audit (same day) found that plan's target_only classifications -- most notably all of June Seq01 -- used the wrong test (target separation/recognisability instead of "no other visible physical person could plausibly explain a controller-facing bbox"); every region in all four sequences was re-examined against the corrected test and none survived it, so docs/issues/p1-10-physical-reference-annotation-plan.md now plans all four sequences as distractors_complete throughout, with interpolation used nowhere, revising the workload to ~1488 expected / ~2954 conservative-upper-bound manual actions (~23%/~46% of total source frames); the same pass corrected a factual error in docs/issues/p1-10-improve-bbox-evaluation.md section F, which had wrongly generalised June's verified 640x480 capture resolution to the May sequence (May's own bag is verified 640x640) -- a provenance-only correction, no schema semantics changed. A read-only feasibility audit (M4A.2) then found a blocking defect: v1's distractors_complete samples cannot interpolate, and the v1 evaluator silently step-holds stale keyframe geometry across the resulting gaps, so the corrected M4A sparse-keyframe plan (now distractors_complete-dominated for all four sequences) cannot produce scientifically valid duration-representative coverage under v1 as implemented. A design audit (M4A.3, read-only) froze a v2 contract (tim_physical_target_bbox_v2, schema_version 2) adding annotation-local physical-person correspondence identifiers (person_ref, never a tracker ID) so distractors_complete can interpolate safely, plus an explicit evaluation horizon and a non-negotiable elimination of the silent stale-geometry hold. M1-v2 landed: docs/issues/p1-10-physical-reference-v2-contract.md freezes the full v2 contract (person_ref namespace/scope, interpolation legality via exact person_ref set match, the frozen reference_gap_duration_s/reference_covered_duration_s/reference_coverage_fraction duration-bucket contract for the not-yet-built M2-v2 evaluator, and UI expectations for the not-yet-built M3-v2), and tools/analysis/physical_target_reference_v2.py is the v2 schema/validator, additive alongside (not replacing) the unmodified tools/analysis/physical_target_reference.py, reusing its stable primitives (bbox validation, Stage A's classify_identity_stage_a unchanged, imported not duplicated) by import, with 56 further focused tests (171 total for #25). M2-v2 landed: tools/analysis/physical_target_bbox_evaluation_v2.py is the interval-aware v2 evaluation core and tools/analysis/evaluate_physical_target_bbox_v2.py its CLI, both additive alongside (not replacing) the unmodified v1 evaluator/CLI -- reference resolution partitions the declared evaluation_window at every sample timestamp so no reference-semantic transition is ever hidden inside one evaluated interval, an isolated keyframe grants exactly zero duration of scored reference by itself, uncovered time (before the first sample, after a present_scored last sample, or between two non-interpolated present_scored keyframes) is honestly bucketed as the new reference_gap_duration_s rather than silently scored, and interpolation (where legal) is per physical person via person_ref, never list position; Stage A/B are reused unchanged from v1 by import, with 41 further focused tests (212 total for #25). M3-v2 landed: the browser "Physical reference" annotation workspace now creates/edits tim_physical_target_bbox_v2 artifacts directly via a new tools/bag_annotation_ui/tim_ui_physical_reference_v2.py backend adapter and tools/bag_annotation_ui/static/tim_physical_reference_v2_ui.js frontend, both additive alongside the unmodified v1 UI code and deferring all schema/validator authority to physical_target_reference_v2.py (never duplicated); a v1-shaped artifact is rejected up front with an explicit message rather than silently migrated or edited; evaluation_window is derived read-only from the loaded bag's own frame timeline (start_s/end_s = first/last frame_times_s), matching the boundary-anchor semantics frozen in 4ab33ec1; person_ref (phys_dNNN) identifiers are annotation-local and never tracker/detector-derived, with "+ New physical person" allocating the lowest currently unused ordinal via matching pure functions in Python and JS; the sole productivity feature is keyboard frame-stepping (ArrowLeft/ArrowRight, suppressed while a form control has focus) driving the existing slider/seek() path so all pre-existing frame-local draft-safety behaviour applies automatically and was extended to the new person_ref state; copy-previous-bbox, tracker/detector-assisted suggestion, optical flow, and auto-propagation were explicitly not implemented; 56 further focused backend/frontend tests (268 total for #25). A human-smoke corrective fix (same day) then found that unchecking "show tracker overlays" was silently discarding the entire unsaved physical-reference draft: the checkbox's onchange called the same function real frame navigation uses, which unconditionally resets frame-local draft state as a navigation safeguard, and toggling the checkbox re-triggered that safeguard with no frame actually changing. Fixed by extracting the shared image-fetch/canvas-buffer mechanics into a draft-state-free helper and giving the checkbox its own pure-repaint handler that reloads the (server-baked) overlay pixels without ever invoking the navigation reset path; real frame navigation is unchanged and still resets the draft exactly as before. 12 further focused tests (280 total for #25). A second human-smoke corrective fix (same day) found that the active-person status chip kept saying "(new, not yet drawn)" for a person_ref that had already been drawn on the current frame, because the chip's "known" check only looked at saved samples, never the current unsaved draft; fixed by deriving the drawn/not-drawn status from the current draft (physicalRefDrawnDistractors) directly, independent of whether the identity is historically known (saved) or brand new, so a drawn box always clears the "not yet drawn" wording immediately with no save/reload/navigation required, and a historically-known identity reused on a later frame is never mislabeled "new". 13 further focused tests (293 total for #25). A third human-smoke corrective fix (same day) found that after a backend-rejected save (changed person_ref set) followed by an explicit "Load selected JSON" of a different, valid two-sample artifact, the sample table correctly reloaded but the canvas/current-frame controls still showed the stale rejected unsaved draft (Target bbox, distractor bboxes, interpolation checked) as though it belonged to the freshly loaded artifact; root cause was that the load path replaced the in-memory sample collection but never resynced the current-frame draft, fixed by having it call the same physicalRefSyncDraftToCurrentFrame() function real frame navigation already uses, so an explicit load now reconstructs the current frame's draft strictly from the loaded artifact (its own saved sample if one exists at this timestamp, otherwise a fully cleared draft) with no autosave. 9 further focused tests (302 total for #25). Not yet done: a v2-based re-plan of the annotation workload (M4A-v2), real sequence annotation, canonical evidence, and human browser re-verification of M3-v2 before it can be frozen.
