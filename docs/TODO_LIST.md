...
1. [x] [#90 — Add long-gap global appearance-based target reacquisition](https://github.com/FRCTavares/IST-Thesis-Code/issues/90) — COMPLETE — DEVELOPMENT EVIDENCE FROZEN
   - Development-only promotion completed on 5 September 2026 without accessing H01--H03 outcomes. Baseline and treatment used identical frozen ByteTrack candidate streams and differed only semantically in `global_reacquisition_enabled=false/true`; no thresholds were tuned. May hard re-entry and Seq01 were controller-facing neutral. Seq03 gained 2.534756980 s correct-target output with 0.133349425 s additional wrong-person exposure; forensic reconstruction showed that the initial global ID-9 reacquisition was valid and the brief error arose about 2.37 s later from tracker drift during the crossing, with TIM detecting the hijack and leaving LOCKED at 46.833011 s. Seq04 gained 8.400856811 s correct-target output with zero wrong-person output and zero output during the 13.900030159 s true target-absence interval. The promoted canonical configuration SHA-256 is `0f2ac3fc780781c3921430310abfddeac2bfeb6c1c833529f2f1054d263f15c0`. Detailed reviewed evidence is frozen in `docs/results/selected_target_tracking/p090_long_gap_global_reacquisition.md`.

   - 7 September 2026 follow-up: the separate [same-ID resilience development study](results/selected_target_tracking/tim_resilience_study_20260907.md) is complete and recommends the available-image challenge for review. Across May/Seq01/Seq03/Seq04 it adds 5.032238257 s correct authority and removes 0.133349425 s wrong authority, with no per-sequence wrong/absence regression; Seq03 alone trades 0.467028962 s correct authority for elimination of its wrong interval. All 20 comparison cells repeat deterministically. Appearance work increases by 27.0% overall, with a larger Seq04 cost documented in the study. Canonical behavior remains unchanged pending review and a new prospective algorithm/configuration/split/comparison freeze before final capture. H01/H02/H03 were not accessed or captured, and PR #99 remains separate.
   - 7 September 2026 architecture-review follow-up: promotion is now intentionally deferred pending a mechanism-level TIM-MARS ablation study. A behavior-neutral Seq03 shadow probe produced the exact reference semantic SHA-256 `307c9c3c2d5ad0f468a452ec8b753552a276e51d3fcd85381158bc13c93811ca` with 14/14 requested frames, showing fresh effective positive support of `0.793--0.887` with positive-minus-negative margins `+0.241--+0.275` after the initial challenged rejection, followed by a negative-margin reversal during the physical handover. This does not prove those suppressed frames were all safely publishable, but it motivates testing state/evidence coupling rather than immediately adding another recovery rule. The completed read-only static audit found 120 scoped interface fields, 56 active canonical algorithmic fields, 44 configured numerical policy fields plus seven active hidden numerical policy sites, and produced an implementation-ready Stage-1 mechanism ablation design. Retained design/evidence: `docs/results/selected_target_tracking/tim_ablation_static_audit_20260907.md` and `docs/results/selected_target_tracking/tim_resilience_development_20260907/seq03_shadow_probe_findings.md`. H01/H02/H03 remain untouched; no new prospective freeze is authorized until the development ablation is reviewed.

2. [ ] [#27 — P0.16 Freeze tuning and test data](https://github.com/FRCTavares/IST-Thesis-Code/issues/27)
   - phase 7; experiment; the active prospective final-evaluation authority is now `tim_mars_split_v3`, re-frozen on 5 September 2026 at algorithm commit `2476991d262f7f388930aece0d731745f20dc1b3` after reviewed promotion of behavior-affecting Issues #89 and #90 and before any H01–H03 capture or outcome access. The matching prospective comparison authority is `tim_mars_final_comparison_v2`. Historical v1 and v2 freezes remain unchanged for provenance. Development/legacy inputs remain development/legacy only. H01/H02/H03 membership and scenarios are unchanged and remain `reserved_pending_capture`; actual source-only capture, physical-v2 annotation, hashing, and participant/clothing-overlap records remain pending. The release gate must stay fail-closed at `final_ready=0/3`, and no held-out outcome may influence thresholds, algorithm logic, tracker settings, model selection, bootstrap rules, or evaluation semantics. Remaining real-world H01–H03 execution is explicitly deferred to `docs/flight/P027_HELDOUT_EXECUTION_PLAN.md`; no physical held-out experiment should be attempted outside the appropriate recording session/environment.
...
**Immediate thesis-critical implementation and evaluation path:** #90 long-gap global appearance reacquisition → TIM-MARS Stage-1 mechanism ablation (complete) → AB-16 source-aware adaptive-memory production promotion → combined AB-16 production regression → #27 prospective held-out freeze/evaluation → #50 closed-loop ground/flight validation → #64 representative drone-POV resolution decision → #58 final held-out closure → #32 final sustained onboard characterisation → #39 final claim freeze. The deterministic #74 controller implementation is complete in software; its remaining physical validation is owned by #50. The pre-#58 #32 instrumentation/evidence gate and the #58 development architecture/embedded-cost comparison completed on 31 August 2026. Issue #58 remains open only for the prospective held-out H01--H03 architecture evidence required after #27; no additional development architecture experiment is currently justified. Issues #25 and #21 are closed. Paused #64 does not block the current #74 deterministic controller work.

**8 September TIM-MARS ablation checkpoint:** the AB-11/AB-18 decomposition remains complete across May, Seq01, Seq03 and Seq04: forced fresh challenge scheduling owns the selected development delta and the later general same-ID negative veto is redundant. The next Stage-1 block is also complete: AB-06 (conservative final appearance filter) is inactive on the available development evidence; AB-09 (hard-negative memory) is diagnostically active but redundant for controller-facing behavior and appearance compute workload on the available development evidence; AB-10 (same-ID positive-support hijack rejection) is safety-essential on Seq03, where disabling it introduces about 1.100 s of wrong-person authority for only about 0.299 s extra correct authority. Seq03 positive-memory smoke ablations now additionally show that removing trusted-gallery storage (AB-07) loses 2.001367555 s of correct authority and removing the adaptive positive representation (AB-08) loses 2.031639277 s, with neither intervention increasing wrong-person authority. Preventing repeated-source adaptive EMA reinforcement (AB-16) is controller-facing neutral on Seq03 while reducing reported trusted adaptive-memory updates from 524 to 124; this is a memory-update simplification result, not an appearance-inference workload reduction. AB-16 now has complete May/Seq01/Seq03/Seq04 development coverage: controller-facing duration buckets and state counts are exactly unchanged in all four sequences while reported trusted adaptive-memory updates fall from 3675 to 1292 overall (64.84%). On the available development evidence, repeated-source adaptive reinforcement is therefore controller-facing redundant and AB-16 is a simplification candidate; this is not an appearance-inference workload reduction. AB-07 and AB-08 now also have complete May/Seq01/Seq03/Seq04 development coverage. Removing trusted-gallery storage loses 0.066 s, 2.001 s and 4.565 s of correct authority on May, Seq03 and Seq04 respectively while Seq01 is neutral; removing the adaptive positive representation is neutral on May/Seq01 but loses 2.032 s and 2.199 s on Seq03/Seq04. Neither ablation increases wrong-person authority on the available development sequences. The positive-memory Stage-1 decision is therefore to retain the protected anchor, trusted gallery and adaptive representation while carrying AB-16 forward as the repeated-source reinforcement simplification candidate. The remaining Stage-1 mechanisms are now also complete. First, the deterministic MARS/TensorFlow replay path was made bitwise reproducible: TF 2.17.1 CPU auto-selects its intra-op thread count from the machine state, so the retained selected semantic SHA-256 digests stopped reproducing (controller-facing output, TIM state and physical-v2 buckets were byte-identical; only ~1e-7 appearance-similarity diagnostics drifted). A pinned environment (TF thread pins + `TF_DETERMINISTIC_OPS=1`; `TF_ENABLE_ONEDNN_OPTS=0` verified non-operative) reproduces its own new semantic SHA-256 3/3 times for May, Seq01 and Seq04 and 11/11 times for Seq03 and is behaviourally identical to the retained selected baseline (physical-v2 buckets, state counts, proposal routing and the `/target` stream all match). The historical semantic SHA-256 values are preserved as historical evidence; the re-baseline is numerical only, not a behavioural or algorithmic re-freeze (`tim_pinned_replay_semantic_rebaseline_20260908.{md,json}`). Second, under that pinned contract: AB-12 (global reacquisition) is availability-essential -- disabling it loses 2.067728018 s correct authority on Seq03 and 9.302666459 s on Seq04 with zero wrong-person or ID-error cost, redundant on May, inactive on Seq01. AB-13 (rank-aware reacquisition), AB-04 (short-gap same-ID priority) and AB-05 (short-gap new-ID suppression) are controller-facing redundant on the permitted sequences -- physical-v2 buckets and state counts are unchanged. AB-14 (min_confirm_frames_after_reacquire = 0) recovers 0.617569768 / 0.066927370 / 0.295821426 s correct authority on May/Seq03/Seq04 with zero new wrong-person: the extra post-reacquisition confirmation frame prevents no wrong publication on the available development evidence and only adds suppression latency, so it joins AB-16 as a simplification candidate. AB-01 (appearance ranking contribution = 0, via appearance_weight, with all appearance gates retained) is controller-facing inactive on all four sequences (Seq01 semantic-identical). Conditional AB-15 (pre-score retirement of over-age committed hard negatives) was executed on Seq03/Seq04 where over-age veto opportunity exists (committed entries up to 863 frames old vs a 247-frame limit) and is INACTIVE -- it changes rejection attribution only, leaving every physical-v2 bucket, state and published `/target` byte-identical, extending the AB-09 finding. Conditional AB-19 (require a distinct source image to advance recovery persistence) was executed on May/Seq03/Seq04 where repeated-source confirmation opportunity exists (5 of 7 May reacquisitions confirm on one cached embedding) and is a HARDENING intervention that removes 2.754305096 / 2.067728018 / 10.066900216 s correct authority with zero new wrong-person or absence leakage -- pure availability cost with no measured safety benefit on development evidence, retained as a diagnostic, not adopted. No ablation created wrong-person authority, absence leakage or memory contamination; no #27 frozen file or held-out sequence was touched. Retained evidence: `tim_ablation_stage1_recovery_ranking_20260908.{md,json}` and `reports/tim_ablation_stage1_pinned_20260908/`. The AB-12/AB-13/AB-04/AB-05/AB-14/AB-01/AB-15/AB-19 runner and algorithm plumbing and the pinned re-baseline are committed on branch `tim-mars-same-id-hijack-resilience-20260907` and retained in PR #100 as the reviewed Stage-1 development checkpoint; this does not constitute canonical simplification promotion or a new prospective freeze. AB-16 and AB-14 are the retained Stage-1 simplification candidates; the full Stage-1 mechanism ablation is now complete and a mechanism-informed TIM-MARS simplification decision plus a new prospective algorithm/config/split/comparison freeze remain before any H01--H03 capture. H01/H02/H03 remain untouched.**

**8 September TIM-MARS Stage-3 architecture decision — AB-16 production promotion:** PR #100 (Stage-1 mechanism-ablation development checkpoint) is merged at `d3edbcc537b92f5ef66771911015e620fc7467d2`. The mechanism-informed simplification decision is now taken and is AB-16 only: source-aware adaptive positive-memory updates are promoted to a canonical `TargetMemoryConfig` / ROS switch (`appearance_prevent_repeated_source_adaptive_update`, generic default `false`, canonical `true`). An eligible adaptive-memory update carrying known source provenance no longer reinforces the adaptive prototype when its source matches the last source already incorporated; last-source suppression only, image timestamp preferred over source-frame fallback, missing provenance preserves existing behaviour, the EMA coefficient, trusted-gallery admission and appearance inference scheduling are unchanged, and the historical development-only AB-16 ablation control is retained for experiment reproduction. AB-14 is **not** promoted: its small development availability gain (0.618 / 0.067 / 0.296 s on May/Seq03/Seq04) does not justify removing the generic post-reacquisition temporal safeguard before prospective evaluation, so `min_confirm_frames_after_reacquire` stays `1`. No other Stage-1 ablation is adopted (AB-15 and AB-19 remain diagnostics only). The new canonical configuration also materialises the selected development baseline exactly (`same_id_fresh_challenge_enabled`, `same_id_challenge_available_images_only`, `appearance_gallery_consensus_recovery_enabled=false`). All retained mechanisms — forced fresh same-ID appearance challenge, available-image challenge, AB-10 same-ID positive-support rejection, global appearance reacquisition, protected anchor, trusted gallery, adaptive positive representation, rank-aware reacquisition, short-gap same-ID priority, short-gap new-ID suppression, appearance-weighted ranking, hard-negative memory and its lifecycle, update-based candidate persistence and generic post-reacquisition confirmation — are unchanged and remain in force through prospective evaluation. Retained decision record: `docs/results/selected_target_tracking/tim_mars_ab16_production_promotion_decision_20260908.md`. The next gate is the combined AB-16 production regression across May/Seq01/Seq03/Seq04 under the pinned replay environment. This is not the Stage-7 prospective freeze; no new prospective algorithm/config/split/comparison freeze has occurred and none is authorised until the combined regression is reviewed. H01/H02/H03 remain untouched.
...
5. [ ] [#58 — P1.13+ Compare lightweight tracker + TIM-MARS against integrated appearance-aware tracking](https://github.com/FRCTavares/IST-Thesis-Code/issues/58) — HIGHEST PRIORITY — IN PROGRESS
   ...
   - 7 September 2026: a development-only ByteTrack configuration sensitivity study is frozen before outcome inspection in `docs/data/tracker_sensitivity/bytetrack_tim_sensitivity_v1.yaml` and `tools/experiments/run_bytetrack_tim_sensitivity.py`. It is a one-factor-at-a-time screen of `track_thresh` (0.40 / 0.60), `match_thresh` (0.70 / 0.90), and `track_buffer` (15 / 45) around the canonical ByteTrack settings, with `new_track_thresh` pinned at 0.6, evaluated on the four `tim_mars_split_v3` development sequences through the identity-independent physical-v2 evaluator against a freshly rerun canonical baseline. It does not rerun or modify the frozen Issue #58 architecture matrices, does not add a tracker, and does not change TIM-MARS or any canonical configuration. The question is whether tuning the tracker propagates to a safer or more available controller-facing TIM-MARS result; the outcome is a recommendation only.
   - 7 September 2026: the ByteTrack configuration sensitivity screen ran from methodology commit `14c3ef20` and completed 36/36 cells with zero bootstrap failures and deterministic baseline repeatability (8/8 checks, zero mismatches). No held-out H01/H02/H03 sequence was used. No candidate configuration produced a meaningful, consistent, safe improvement in the controller-facing TIM-MARS result: `track_thresh_lower` (0.40) is neutral; `track_thresh_higher` (0.60) is an unsafe regression, adding `+0.451 s` of TIM wrong-target authority on May hard re-entry; `match_thresh_lower` (0.70) regressed, losing `1.034 s` of correct-target authority on Seq04; `match_thresh_higher` (0.90), `track_buffer_lower` (15) and `track_buffer_higher` (45) are neutral downstream. On Seq03, `match_thresh_higher` inflates raw fixed-ID ByteTrack wrong-person output from `0.100 s` to `22.798 s` while TIM-MARS output remains byte-identical to baseline, evidence that TIM-MARS absorbs severe upstream association failure within these parameters and ranges. No second-pass combination sweep was justified. Recommendation: NO -- development evidence does not justify changing the canonical ByteTrack configuration, and `ros2_ws/src/thesis_bringup/config/tracker_bytetrack.yaml` is unchanged. Evidence: `docs/results/selected_target_tracking/p058b_bytetrack_config_sensitivity.md` and `docs/results/selected_target_tracking/p058b_bytetrack_config_sensitivity_development/`.
...
4. [x] [#49 — P2: Consolidate replay bags and define evidence retention policy](https://github.com/FRCTavares/IST-Thesis-Code/issues/49)
   ...
   - 5 September 2026 current-state audit: `bags/` is approximately 72 GiB (`replay` approximately 39 GiB, `source` approximately 10 GiB, `reference` approximately 427 MiB); the historical `bags/annotation_inputs/` and `bags/review/` trees are absent. Seven dangling aliases left by the July cleanup were verified against the tracked deletion record; the real Seq01/Seq04 curated source bags remain present. The approximately 14.3 GiB P030 external/oracle replay families are retained because tracked final-result reproduction and analysis tooling depend on them. No bulk replay deletion is justified by this audit; remaining #49 work is the current retention policy/manifest and final verification.
   - 5 September 2026 retention authority: `docs/data/catalogue/evidence_retention_policy.md` and `docs/data/catalogue/evidence_retention_manifest_2026_09_05.json` now define the conservative deletion gate, classify all 76 current top-level replay families, and inventory all 310 replay bags plus the single reference bag in the original #49 scope. `bags/annotation_inputs/` and `bags/review/` are absent. The Git object pack remains approximately 4.52 GiB; model artifacts retain their current tracked/untracked state because provenance recovery is incomplete, and #49 performs no history rewrite or artifact migration. No current bag passed the deletion gate, so this pass removes no bag data. Final verification passed on 5 September 2026; #49 is complete pending merge/issue closure.
