...
1. [ ] [#89 — Fix wide-crop appearance eligibility blocking TIM-MARS reacquisition](https://github.com/FRCTavares/IST-Thesis-Code/issues/89) — CLOSURE READY — LIVE + FINAL SAFETY PASS
   - immediate algorithm gate before #90 and before prospective held-out H01--H03 evaluation. The 1 September live reproduction showed a large, unclipped same-ID crop being rejected before MARS encoding, leaving LOST reacquisition suppressed as `same_id_reacquisition_reject:no_candidate_appearance`.
   - the final contract separates current comparison/encoding eligibility from trusted positive-memory update eligibility. Wide valid crops may provide fresh identity evidence while the canonical `appearance_crop_max_aspect_ratio=1.00` remains a positive-memory trust gate. Same-ID hijack protection remains enabled, no geometry-only fallback was added, and overlap/group-contaminated or otherwise unsafe comparison crops remain rejected from protected-gallery recovery.
   - final software state is commit `64cce622`. Test-first protected-gallery coverage passed; the complete `thesis_bringup` suite records `427 passed, 1 skipped`; `tools/thesis_build.sh --packages-select thesis_bringup` passed; fatal Python lint and `git diff --check` passed.
   - fresh live acceptance on 4 Sep from clean commit `64cce622` passed on the canonical YOLOv8s + ByteTrack + CPU-MARS stack with control disabled. A trusted narrow ID `1` selection built a protected anchor and gallery of `4`, retained the same tracker ID into a wide pose, entered `LOST` after a controlled lens cover, then completed `LOST -> REACQUIRED -> LOCKED`. The wide crop remained `encoding_eligible=true`, `memory_update_eligible=false`, with sole reason `aspect_ratio_too_wide`; `260/260` observed wide samples satisfied the comparison-only contract, `48` carried evaluated appearance, positive-memory updates were `0`, and wrong visible authority was `0`.
   - final deterministic Seq03/Seq04 replays at `64cce622` preserved the frozen candidate-stream hashes and exactly preserved the earlier generated semantic digests. Final physical-v2 duration buckets are unchanged: Seq03 TIM wrong-person output `0 s`; Seq04 TIM wrong-person output `0 s` and target-absence-with-output `0 s`. H01--H03 remain untouched.
   - tracked evidence: `docs/results/selected_target_tracking/p089_wide_crop_reacquisition.md`. All Issue #89 technical acceptance gates are satisfied; only PR merge / GitHub closure and roadmap reconciliation remain. Do not start #90 until #89 is merged and closed.
