...
- [Copied lock file, provenance, batch logs, aggregate tables](p031_parameter_sensitivity_development/)
...
Figures (git-ignored generated output, like the P030 bbox-size figures --
regenerate with `tools/analysis/plot_parameter_sensitivity.py`):
`reports/p031_parameter_sensitivity_5b340c2b_2026-08-08/aggregate/figures/p031_all_dimensions_wrong_lost.png`,
`reports/p031_parameter_sensitivity_5b340c2b_2026-08-08/aggregate/figures/p031_confirmation_time_tradeoff.png`.
...
logs, and the aggregate CSV/JSON tables are copied byte-for-byte into
...
git-ignored generated output (the repository's `figures/` gitignore rule
applies here the same way it applies to P030's bbox-size figures) and are
not duplicated into version control; they are regenerated by
`tools/analysis/plot_parameter_sensitivity.py` from the tracked
`matrix_aggregate.csv` alone. `run_provenance.json`
