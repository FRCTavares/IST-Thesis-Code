...
| H1 frontend hardening | 2026-09-02 | N/A | `ebe85267e9c38015d93eff88279328e42183b05b` | PASS | external UI now has one canonical WebSocket provider; deterministic `format`, `lint`, `typecheck`, `test`, `build`, generated-artifact, launcher-smoke, and aggregate `npm run verify` contracts; GitHub Actions runs locked `npm ci` plus `npm run verify`; clean-install validation PASS with 7/7 Vitest tests, production build PASS, generated-source-tree guard PASS, and deterministic production-launch/shutdown smoke PASS. Four React exhaustive-deps warnings and the transitive Recharts/lodash audit finding are recorded non-blocking follow-up findings. |
| H2 backend hardening | 2026-09-02 | `ad08e8ef15b9346011e7904bbbe46f35a9d26fa3` | `9bbdb0be1c18b34fe14e5626a35d876874e4f8f3` | PASS | conservative loopback API/WS defaults; wildcard CORS removed; unapproved browser Origins rejected before handler execution; field remote access remains explicitly scoped to Pixhawk-gated wlan0, exact frontend Origin and wlan0-only UFW; frozen model/tracker reconfiguration protection retained; API/authority/service-failure and WebSocket status contracts covered by tests. MAVROS BatteryState telemetry added as informational-only browser data with explicit local receive-age freshness and no voltage-derived percentage. Real Pixhawk/MAVROS and full field-stack behavior remain subject to ground validation before flight. External UI battery presentation retained at `9bbdb0be1c18b34fe14e5626a35d876874e4f8f3`; 11/11 frontend tests pass, including 4/4 battery presentation contracts. Real Pixhawk/MAVROS and complete field-stack behavior remain subject to ground validation before flight. |
