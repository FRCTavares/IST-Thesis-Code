...
activate_field_wifi() {
    ...
    for candidate in "$PIXHAWK_WIFI" "$PIXHAWK_WIFI_FALLBACK"; do
        ...
        if [ "$active_wifi" = "$candidate" ]; then
            echo "[field] Wi-Fi profile already active: $candidate" >&2
            echo "$candidate"
            return 0
        fi
        ...
        if nmcli connection up "$candidate" ifname "$INTERFACE" >/dev/null; then
            active_wifi="$(
...
case "$MODE" in
    ...
    pixhawk)
        ...
        if verify_pixhawk_ethernet_state "$pixhawk_interface" \
            >/dev/null 2>&1; then
            echo "[field] Pixhawk Ethernet profile already active and valid"
        else
            nmcli connection up "$PIXHAWK_ETHERNET" \
