#!/usr/bin/env bash
# Controlled Issue #44 guarded Hailo ReID load matrix.
#
# Conditions:
#   reference
#     Detector only. Async RepVGG transport disabled.
#     CPU request policy remains all_candidates at 250 ms.
#
#   all_candidates_hailo
#     Detector plus asynchronous RepVGG transport.
#     CPU request policy is all_candidates at 250 ms.
#
#   ambiguity_guarded_hailo
#     Detector plus asynchronous RepVGG transport.
#     CPU request policy is ambiguity_guarded at 250 ms.
#
# CPU MARS remains authoritative. RepVGG ranking, memory, cache,
# and target-decision integration remain disabled. The canonical
...
TAG="p044_guarded_hailo_load_${SHORT_HEAD}_${DATE_TAG}_${RUN_NAME}"
...
{
  "schema": "p044_guarded_hailo_load_matrix_v1",
  "git_head": "$HEAD",
  "source_bag": "$BAG_PATH",
...
run_condition() {
  ...
  printf '\nRunning condition=%s repetition=%s interval_ms=%s policy=%s\n' \
    "$condition" \
    "$repetition" \
    "$interval_ms" \
    "$appearance_policy"
...
{
  "schema": "p044_guarded_hailo_load_run_v1",
  "git_head": "$HEAD",
  "condition": "$condition",
...
for repetition in $(seq 1 "$REPETITIONS"); do
  ...
  case "$rotation" in
    0)
      order="reference all_candidates_hailo ambiguity_guarded_hailo"
      ;;
    1)
      order="all_candidates_hailo ambiguity_guarded_hailo reference"
      ;;
    2)
      order="ambiguity_guarded_hailo reference all_candidates_hailo"
      ;;
  esac
...
output = {
    ...
    "comparisons": {
        "all_candidates_hailo_vs_reference": comparison(
            "all_candidates_hailo",
            "reference",
        ),
        "ambiguity_guarded_hailo_vs_reference": comparison(
            "ambiguity_guarded_hailo",
            "reference",
        ),
        "ambiguity_guarded_hailo_vs_all_candidates_hailo": comparison(
            "ambiguity_guarded_hailo",
            "all_candidates_hailo",
        ),
    },
...
if [ "$overall_status" -eq 0 ]; then
  printf 'PASS: controlled three-condition guarded Hailo load matrix completed.\n'
  printf 'Resource evidence covers complete TIM and perception process groups.\n'
  printf 'CPU MARS remained authoritative; ambiguity_guarded changed only pre-embedding request selection.\n'
else
