...
def simulate(
    ...
    for frame, rows in by_frame.items():
        ...
        # Arm contradiction suppression only after instability.
        # This avoids suppressing long stable correct LOCKED intervals.
        if state in {"UNCERTAIN", "LOST", "REACQUIRED"}:
            arm_countdown = max(arm_countdown, int(arm_after_instability_frames))
        elif arm_countdown > 0:
            arm_countdown -= 1

        # Only suppress LOCKED output while armed.
        if state == "LOCKED" and current_id > 0 and arm_countdown > 0:
...
def main():
    ...
    ap.add_argument("--arm-after-instability-frames", type=int, default=60)
    args = ap.parse_args()
    ...
    outputs = simulate(
        ...
        arm_after_instability_frames=args.arm_after_instability_frames,
    )
    ...
    print("# TIM-V2M Armed Locked Suppression")
    print()
    print("| Metric | Value |")
    print("|---|---:|")
