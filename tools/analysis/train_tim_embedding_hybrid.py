#!/usr/bin/env python3
"""
Train TIM-V2E Tiny16 hybrid embedding.

Prototype only:
- binary classification keeps strong target/distractor separation
- triplet loss encourages metric-like embedding structure
- offline only, does not modify live TIM
"""
...
class HybridTripletSampler:
    def __init__(self, samples: list[Sample], negative_time_window_s: float = 1.0):
        ...
        if len(self.correct) < 2:
            raise RuntimeError("Need at least two correct samples.")
        if not self.distractor:
            raise RuntimeError("Need distractor samples.")

    def sample(self, batch_size: int) -> tuple[list[Sample], list[Sample], list[Sample]]:
        anchors = []
        positives = []
        negatives = []

        for _ in range(batch_size):
            a = random.choice(self.correct)

            pos_pool = [
                s for s in self.correct
                if s.path != a.path and abs(float(s.t) - float(a.t)) > 0.5
            ] or [s for s in self.correct if s.path != a.path] or self.correct

            neg_pool = [
                s for s in self.distractor
                if s.dataset_root == a.dataset_root
                and abs(float(s.t) - float(a.t)) <= self.negative_time_window_s
            ] or [
                s for s in self.distractor
                if s.dataset_root == a.dataset_root and s.event == a.event
            ] or [
                s for s in self.distractor
                if s.dataset_root == a.dataset_root
            ] or self.distractor
...
def load_crop_batch(samples: list[Sample], augment: bool, device: torch.device) -> torch.Tensor:
    xs = []
    ds = CropDataset(samples, augment=augment)
...
def write_report(path: Path, metrics: dict, args, train_n: int, test_n: int) -> None:
    ...
    lines.append(f"- Triplet weight: {args.triplet_weight}")
    lines.append(f"- Triplet margin: {args.triplet_margin}")
...
def parse_args():
    ...
    p.add_argument("--triplet-margin", type=float, default=0.4)
...
def main() -> int:
    ...
    triplet_sampler = HybridTripletSampler(train_samples, negative_time_window_s=args.negative_time_window_s)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for x, y, _role, _event, _path in train_dl:
            ...
            a_s, p_s, n_s = triplet_sampler.sample(batch_size=int(x.shape[0]))
            xa = load_crop_batch(a_s, augment=True, device=device)
            xp = load_crop_batch(p_s, augment=True, device=device)
            xn = load_crop_batch(n_s, augment=True, device=device)
            ...
            tri = F.relu(pos_dist - neg_dist + args.triplet_margin).mean()
            ...
            tri_losses.append(float(tri.item()))
    ...
    torch.save(
        {
            ...
            "input_size": [64, 128],
            "note": "TIM-V2E prototype hybrid CE+triplet embedding. Offline only.",
        },
