...
def simulate(
    ...
    for frame, rows in sorted(by_frame.items()):
        ...
        # LOST-state appearance-confirmed reacquisition.
        if selected_id == 0:
            best = best_candidate(cands, "total")
            if best is not None:
                ...
                ok = (
                    best["total"] >= lost_min_total
                    and best["geom"] >= lost_min_geom
                    and best["app"] >= lost_min_app
                    and app_advantage >= lost_app_margin
                )
...
def main():
    ...
    md = []
    md.append("# TIM-V2I Appearance-Confirmed LOST Reacquisition")
    md.append("")
