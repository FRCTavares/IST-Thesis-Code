...
def find_metadata_bags(base: Path) -> list[str]:
    roots = [
        base / "artifacts/bags/ui_replays",
        base / "artifacts/bags/eval_matrix",
...
def run_replay_job(req: ReplayRequest):
    ...
    env.update({
        ...
        "TIM_REPLAY_OUT_ROOT": str(ROOT / "artifacts/bags/ui_replays"),
...
function emergencyBagInfo(path) {
  ...
  let prefix = "bag";
  if (lower.includes("/artifacts/bags/ui_replays/")) prefix = "ui";
  else if (lower.includes("/artifacts/bags/datasets/")) prefix = "dataset";
  else if (lower.includes("/artifacts/bags/eval_matrix/")) prefix = "eval";
