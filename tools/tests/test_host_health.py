...
def test_pixhawk_disconnect_service_revalidates_after_serialized_transition():
    service = (
        SYSTEMD_ASSET_ROOT / "thesis-pixhawk-disconnect.service"
    ).read_text(encoding="utf-8")
    mode_script = (
...
def test_field_wifi_activation_stdout_is_only_the_resolved_profile():
    mode_script = (
        REPO_ROOT / "tools/host/set_pi_network_mode.sh"
    ).read_text(encoding="utf-8")

    activation = (
        'nmcli connection up "$candidate" ifname "$INTERFACE" >/dev/null'
    )
    assert activation in mode_script

    function = mode_script[
        mode_script.index("activate_field_wifi() {"):
        mode_script.index("maybe_start_health_check() {")
    ]
    assert 'echo "$candidate"' in function
    assert activation in function


def test_pixhawk_mode_does_not_cycle_already_valid_field_links():
    mode_script = (
        REPO_ROOT / "tools/host/set_pi_network_mode.sh"
    ).read_text(encoding="utf-8")

    assert "Wi-Fi profile already active" in mode_script
    assert "Pixhawk Ethernet profile already active and valid" in mode_script
    assert (
        'if verify_pixhawk_ethernet_state "$pixhawk_interface"'
        in mode_script
    )
