...
| `operator_event.py` | Operator events | Append-only JSONL recorder for physical-trial operator events (trial start/end, target selected, takeover, abort + reason, unexpected behavior, trial verdict), tied to `RUN_ID`. |
