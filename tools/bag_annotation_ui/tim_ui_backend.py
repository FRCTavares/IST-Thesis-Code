...
def find_metadata_bags(base: Path) -> list[str]:
    roots = [
        base / "bags/replay/ui_replays",
        base / "bags/replay/eval_matrix",
        base / "bags/replay/conservative_safety_eval",
        base / "bags/live_camera",
        base / "bags/datasets",
        base / "bags/source_video",
    ]
    out = []
    for r in roots:
        if not r.exists():
...
function emergencyBagInfo(path) {
  ...
  let prefix = "bag";
  if (lower.includes("/bags/replay/ui_replays/")) prefix = "ui";
  else if (lower.includes("/bags/datasets/")) prefix = "dataset";
  else if (lower.includes("/bags/replay/conservative_safety_eval/")) prefix = "safe";
  else if (lower.includes("/bags/replay/eval_matrix/")) prefix = "eval";
  else if (lower.includes("/bags/live_camera/")) prefix = "live";
  else if (lower.includes("/bags/source_video/")) prefix = "source";
