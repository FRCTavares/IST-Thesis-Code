...
<body>
  ...
  <div class="panel">
    ...
    <div id="controls">
      ...
      <button onclick="addCurrentFrameToPaperList()" title="Append current frame to paper contact-sheet frame list">+ paper frame</button>
      <button onclick="clearPaperFrameList()" title="Clear paper contact-sheet frame list">clear frames</button>
      <span id="paperFrameDraft" style="font-family:monospace;color:#91f5aa;">frames: none</span>
    ...
    <div class="bagFinderCard" style="max-width:1320px;margin:10px 0;padding:12px;border:1px solid #333;border-radius:10px;background:#171717;">
...
<details open>
      ...
      <div class="controls">
        <label>Video name
          <input id="videoName" value="paper_video_export.mp4" style="min-width:360px">
        </label>
        <label>FPS
...
function paperNeedEmoji(path) {
  ...
  if (l.includes("paper_v5_bytetrack_seq03_target1")) return "🎯";
  if (l.includes("paper_v5_bytetrack_seq04_target5")) return "🎯";
  ...
  if (l.includes("paper_final") || l.includes("paper_v") || l.includes("controlled_v5")) return "🧪";
...
function renderBagOptions(filterText = "") {
  ...
  console.log("Bag filter:", f || "[none]", "shown:", bags.length, "total:", allBags.length);
}
...
function defaultVideoNameForBag() {
  const b = loadedBag || document.getElementById("bag").value || "paper_video_export";
  const base = b.split("/").pop()
    .replace(/^VIDEO__/, "")
    .replace(/[^A-Za-z0-9_.-]+/g, "_")
    .replace(/_+$/g, "");
  return (base || "paper_video_export") + ".mp4";
}
...
async function exportMp4() {
  ...
  if (!name || name === "paper_video_export.mp4") {
  ...
  const payload = {
    out: "reports/paper_final_videos_2026_07_04/" + name,
