...
def draw_panel(img, title: str, reason: str, source_t_s: float, eval_t_s: float, target_id: int, box):
    out = img.copy()
    ...
    status = f"source_t={source_t_s:5.2f}s eval_t={eval_t_s:6.2f}s id={target_id} {reason}"
    cv2.putText(out, status, (20, 66), cv2.FONT_HERSHEY_SIMPLEX, 0.56, (255, 255, 255), 3, cv2.LINE_AA)
    cv2.putText(out, status, (20, 66), cv2.FONT_HERSHEY_SIMPLEX, 0.56, (0, 0, 0), 1, cv2.LINE_AA)
...
def read_eval_tracks(eval_bag: Path, tracks_topic: str):
    ...
    msg_type = get_message(topic_types[tracks_topic])
    events = []
    first_ns = None
    ...
    while reader.has_next():
        ...
        events.append((t_s, extract_tracks(msg)))
...
def main() -> int:
    ...
    eval_tracks = read_eval_tracks(args.eval_bag, args.tracks_topic)
    eval_idx = 0
    latest_tracks = {}
    ...
    while reader.has_next():
        ...
        while eval_idx + 1 < len(eval_tracks) and eval_tracks[eval_idx + 1][0] <= eval_t_s:
            eval_idx += 1
        if eval_tracks:
            latest_tracks = eval_tracks[eval_idx][1]

        img = image_msg_to_bgr(deserialize_message(raw, image_type))
        target_id, reason = annotation_at(ann, eval_t_s)
        ...
        panel = draw_panel(img, args.title, reason, source_t_s, eval_t_s, target_id, box)
