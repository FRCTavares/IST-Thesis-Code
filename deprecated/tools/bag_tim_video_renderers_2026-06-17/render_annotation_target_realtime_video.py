...
def main() -> int:
    ...
    while reader.has_next():
        ...
        if topic == args.tracks_topic:
            ...
            while next_out_t <= t_s:
                target_id, reason = annotation_at(ann, next_out_t)
                box = None
                ...
                panel = draw_panel(latest_img, args.title, reason, next_out_t, target_id, box)
                panel = cv2.resize(panel, (out_w, out_h), interpolation=cv2.INTER_LINEAR)
                writer.write(panel)

                rendered += 1
                next_out_t += 1.0 / args.fps

                if rendered % 250 == 0:
                    print(f"[info] rendered {rendered} frames")

    if writer is not None and latest_img is not None:
        while next_out_t <= last_t_s:
            target_id, reason = annotation_at(ann, next_out_t)
            box = None
            if target_id > 0 and target_id in latest_tracks:
